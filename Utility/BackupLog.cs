...
namespace Utility
{
    public class BackupLogger : IDisposable
    {
        ...
        private static readonly string _logDir = Parameters.BackupPath;
        private StreamWriter _logSW;

        private static BackupLogger _logger;
        ...
        public static BackupLogger Instance
        {
            get
            {
                lock (_lockObject)
        ...
        private BackupLogger()
        {
            ...
            Directory.CreateDirectory(_logDir + @"\Backup");
        ...
        public void Open()
        {
            ...
            try
            {
                ...
                if (File.Exists(_logDir + @"\Backup.txt"))
                {
                    oldFile = String.Format(@"{0}\Backup_{1}.log", _logDir, DateTime.Now.ToString("yyMMdd-HHmmss"));
                    File.Move(_logDir + @"\Backup.txt", oldFile);
                }

                this._logSW = File.CreateText(_logDir + @"\Backup.txt");
                this._logSW.AutoFlush = true;
                this._logSW.WriteLine("*** New Backup Created - Last Backup: {0} ***", oldFile);
            }
        ...
        public void Close()
        {
            ...
            try
            {
                if (_logSW != null)
                {
                    _logSW.WriteLine("*** Closing Backup ***");
                    _logSW.Flush();
                    _logSW.Dispose();
                }
            }
